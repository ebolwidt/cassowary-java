...
public class AlignmentConstraint extends Constraint {
    ...
    public void replaceSelPoint(SelPoint oldsp, SelPoint newsp) {
         ...
         * "removing #" + oldIdx + " = " + cle); }
         * 
         * // Create a new constraint to take its place try { cli = new ClLinearInequality(newsp.X(), CL.LEQ, srcCC.topSP.Y()); }
         * catch (ExCLInternalError e) { System.out.println("AlignConstr.repSP: ExCLInternalError replacing #" + oldIdx); }
         * relConstrs.setElementAt(cli, oldIdx); try { if ( cli != null ) solver.addConstraint(cli); } catch (ExCLInternalError e)
         * { System.out.println("AlignConstr.repSP: ExCLInternalError adding #" + oldIdx + " = " + cli); } catch
         * (ExCLRequiredFailure e) { System.out.println("AlignConstr.repSP: ExCLRequiredFailure " + "adding #" + oldIdx + " = " +
         * cli); } }
         */
    }
...
/*
 * $Log: AlignmentConstraint.java,v $ Revision 1.4 1999/12/16 02:10:00 gjb * java/cda/Makefile.am: Put Constraint/*, Main/* files
 ...
 * 
 * Revision 1.3 1998/06/23 02:08:45 gjb Added import of cassowary package so that the cda doesn't need to be in the same package
 * as the solver.
 * 
